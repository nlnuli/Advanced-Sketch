    ...
    apply {
        if(hdr.secret.isValid()) {
            ...
            /* Hint 1: verify if the secret message is destined to the switch */
             if(hdr.ipv4.dstAddr == SWITCH_IP && hdr.udp.dstPort == TYPE_SECRET && hdr.udp.srcPort == TYPE_SECRET){
            /* Hint 2: there are two cases to handle -- DROPOFF, PICKUP */
                if(hdr.secret.opCode == SECRET_OPT.DROPOFF){
                    //DROPOFF
                    //secret_mailboxes.read(hdr.secret.message,(bit<32>)hdr.secret.mailboxNum);
                    secret_mailboxes.write((bit<32>)hdr.secret.mailboxNum,hdr.secret.message);
                    bit<32> hashTemp;
                    hash(hashTemp,HashAlgorithm.crc32, (bit<32>)0, {hdr.secret.message}, (bit<32>)0xffffffff);
                    msg_checksums.write((bit<32>)hdr.secret.mailboxNum,hashTemp);
                    // seccess
                    ...
                    msg_checksums.read(msgTemp,(bit<32>)hdr.secret.mailboxNum);
                    if(msgTemp == 0 ){
                        //empty
                        ...
                        bit<32> tempMessage;
                        bit<32> tempChecksum;
                        bit<32> tempChecksumOfBox;
                        secret_mailboxes.read(tempMessage,(bit<32>)hdr.secret.mailboxNum);
                         /* Hint 5: msg_checksums are important! */
                        hash(tempChecksum,HashAlgorithm.crc32, (bit<32>)0, {tempMessage}, (bit<32>)0xffffffff);
                        msg_checksums.read(tempChecksumOfBox,(bit<32>)hdr.secret.mailboxNum);
                        if(tempChecksum == tempChecksumOfBox){
                            hdr.secret.opCode = SECRET_OPT.SUCCESS;
                            hdr.secret.message = tempMessage;
                             /* Hint 4: remember to "sanitize" your mailbox with 0xdeadbeef after every PICKUP */
                            secret_mailboxes.write((bit<32>)hdr.secret.mailboxNum, 0xdeadbeef);
                            msg_checksums.write((bit<32>)hdr.secret.mailboxNum, 0x0);
                        }
                        else{
                            hdr.secret.opCode = SECRET_OPT.FAILURE;
                        }
                ...
                bit<32> tem;
                tem = hdr.ipv4.srcAddr;
                hdr.ipv4.srcAddr = hdr.ipv4.dstAddr;
                hdr.ipv4.dstAddr = tem;
                bit<16> tempPort;
